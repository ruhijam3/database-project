...
<script lang="ts">
    ...
    async function remove(id: string) {
      ...
      try {
        const r = await fetch(`${API}/location/${id}`, { method: "DELETE" });
        if (!r.ok) throw new Error(await r.text());
        await load();
  ...
  <div class="page">
    <div class="card">
      ...
      <form class="form" on:submit|preventDefault={addLocation}>
        <label>
          <span>Name:</span>
          <input bind:value={name} placeholder="Giza Necropolis" required />
        </label>
        <label>
          <span>Street:</span>
        ...
        <label>
          <span>Latitude:</span>
          <input type="number" step="any" bind:value={latitude} placeholder="29.9792" required />
        </label>
        <label>
          <span>Longitude:</span>
          <input type="number" step="any" bind:value={longitude} placeholder="31.1342" required />
        </label>
    ...
    <div class="list">
      ...
      {:else}
        <table>
          <thead>
            <tr><th>Name</th><th>Street</th><th>City</th><th>Zip</th><th>Lat</th><th>Lng</th><th></th></tr>
          </thead>
          <tbody>
            {#each rows as r}
              <tr>
                ...
                <td>{r.latitude}</td>
                <td>{r.longitude}</td>
                <td><button class="btn del" on:click={() => remove(r.id)}>Delete</button></td>
              </tr>
            {/each}
          </tbody>
        </table>
